...
public class RelatorioService {
    ...
    public List<ChamadoEntity> buscarDadosRelatorio(RelatorioFiltroInput filtro, UsuarioEntity usuarioLogado) {
        aplicarRegraDeVisibilidade(filtro, usuarioLogado);
        return chamadoRepository.findAll(ChamadoSpecification.comFiltros(filtro));
    }
    ...
    public RelatorioGeradoOutput gerarExcel(RelatorioFiltroInput filtro, UsuarioEntity usuarioLogado) throws Exception {
        ...
        return new RelatorioGeradoOutput(nome, bytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    }
    ...
    private String calcularDuracao(ChamadoEntity c) {
        if (c.getDataFechamento() == null) return "Aberto";
    ...
    private byte[] gerarPdfBytes(List<ChamadoEntity> chamados, UsuarioEntity geradoPor) throws Exception {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            HeaderFooter footer = new HeaderFooter(new Phrase("Página ", new com.lowagie.text.Font(com.lowagie.text.Font.HELVETICA, 8, com.lowagie.text.Font.NORMAL, Color.GRAY)), true);
            ...
            Paragraph meta = new Paragraph("Gerado por " + geradoPor.getNome() + " em " + LocalDateTime.now().format(DATE_FMT), metaFont);
            ...
            com.lowagie.text.Font dataFont = new com.lowagie.text.Font(com.lowagie.text.Font.HELVETICA, 9, com.lowagie.text.Font.NORMAL, Color.DARK_GRAY);
            ...
            for (ChamadoEntity c : chamados) {
                ...
                if(PrioridadeEnum.CRITICA.equals(c.getPrioridade())) {
                    rowFont = new com.lowagie.text.Font(com.lowagie.text.Font.HELVETICA, 9, com.lowagie.text.Font.BOLD, new Color(192, 57, 43));
                }
    ...
    private byte[] gerarExcelBytes(List<ChamadoEntity> chamados, UsuarioEntity geradoPor) throws Exception {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            CellStyle styleRowReaberto = criarEstiloDadosExcelColorido(workbook, IndexedColors.CORAL, HorizontalAlignment.LEFT);
            CellStyle styleRowReabertoCentro = criarEstiloDadosExcelColorido(workbook, IndexedColors.CORAL, HorizontalAlignment.CENTER);
            
            CellStyle styleRowConcluido = criarEstiloDadosExcelColorido(workbook, IndexedColors.LIGHT_GREEN, HorizontalAlignment.LEFT);
            CellStyle styleRowConcluidoCentro = criarEstiloDadosExcelColorido(workbook, IndexedColors.LIGHT_GREEN, HorizontalAlignment.CENTER);
            ...
            String[] colunas = { 
                "Protocolo", "Status", "Prioridade", "Título", "Descrição Resumida", 
                "Solicitante", "Empresa", "Técnico", 
                "Abertura", "Atualização", "Fechamento", "Duração", 
                "Nota", "Comentário" 
            };
            ...
            for (ChamadoEntity c : chamados) {
                ...
                String desc = (c.getDescricao() != null && c.getDescricao().length() > 50) 
                        ? c.getDescricao().substring(0, 50) + "..." : c.getDescricao();
                ...
                criarCelula(row, 7, c.getTecnicoResponsavel() != null ? c.getTecnicoResponsavel().getNome() : "-", currentStyleTexto);
                ...
                criarCelula(row, 12, c.getAvaliacaoNota() != null ? c.getAvaliacaoNota().toString() : "-", currentStyleCentro);
    ...
    private CellStyle criarEstiloDadosExcelColorido(Workbook workbook, IndexedColors corFundo, HorizontalAlignment align) {
